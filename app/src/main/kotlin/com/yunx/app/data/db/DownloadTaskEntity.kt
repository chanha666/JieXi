...
data class DownloadTaskEntity(
    ...
    @ColumnInfo(defaultValue = "''") val sourceLink: String = "",
    @ColumnInfo(defaultValue = "''") val shareId: String = "",
    @ColumnInfo(defaultValue = "''") val fileId: String = "",
    ...
    @ColumnInfo(defaultValue = "'BALANCED'") val preset: String = "BALANCED",
    @ColumnInfo(defaultValue = "0") val updatedAt: Long = System.currentTimeMillis(),
    /** Author-signed expected hash for an in-app update APK; blank for ordinary downloads. */
    @ColumnInfo(defaultValue = "''") val expectedSha256: String = ""
) {
    companion object {
        ...
        const val STATUS_PREPARING = 5
        const val STATUS_RETRY_WAIT = 6
        const val STATUS_INTERRUPTED = 7
        const val STATUS_NEEDS_REAUTH = 8
        const val STATUS_NEEDS_INPUT = 9
        const val STATUS_VERIFYING = 10
        const val STATUS_CANCELLED = 11
        ...
        fun statusText(status: Int): String = when (status) {
            ...
            STATUS_INTERRUPTED -> "意外中断"
            STATUS_NEEDS_REAUTH -> "需要重新登录"
            STATUS_NEEDS_INPUT -> "需要处理"
            STATUS_VERIFYING -> "正在校验"
