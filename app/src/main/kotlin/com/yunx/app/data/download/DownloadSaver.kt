...
object DownloadSaver {
    ...
    fun save(
        ...
        if (!targetDirUri.isNullOrBlank()) {
            val saved = saveViaSaf(context, safeName, safeDir, source, targetDirUri, shouldContinue)
            return verifyStillWanted(context, saved, shouldContinue)
        }
        // 默认目录：Android 10+ 优先 MediaStore；失败则回退传统文件路径（Android 9- 可用）
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            saveViaMediaStore(context, safeName, safeDir, source, shouldContinue)?.let {
                return verifyStillWanted(context, it, shouldContinue)
            }
            Log.e(TAG, "MediaStore 保存失败，回退传统路径：$safeDir/$safeName")
        }
        saveLegacy(context, safeName, safeDir, source, shouldContinue)?.let {
            return verifyStillWanted(context, it, shouldContinue)
        }
        ...
        return runCatching {
            ...
            for (candidate in candidates) {
                ...
                try {
                    ...
                    docUri = created
                    val wrote = resolver.openOutputStream(created)?.use { out ->
                        source.inputStream().use { copyCancellable(it, out, shouldContinue) }
                        true
                    } ?: false
                    ensureContinue(shouldContinue)
                    if (wrote) {
                    ...
                    if (!committed) docUri?.let { runCatching { resolver.delete(it, null, null) } }
                }
            }
            null
    ...
    private fun saveViaMediaStore(
        ...
        for (candidate in candidates) {
            ...
            try {
                ...
                val created = resolver.insert(MediaStore.Downloads.EXTERNAL_CONTENT_URI, values) ?: continue
                uri = created
                val wrote = resolver.openOutputStream(created)?.use { out ->
                    source.inputStream().use { copyCancellable(it, out, shouldContinue) }
                    true
                ...
                if (resolver.update(created, values, null, null) <= 0) {
                    throw IOException("MediaStore 无法提交下载文件")
                }
                ensureContinue(shouldContinue)
                committed = true
                return created.toString()
                ...
                // Includes disk-full, revoked permission and cancellation. Never
                // leave an IS_PENDING row or a multi-GB partial candidate behind.
                if (!committed) uri?.let { runCatching { resolver.delete(it, null, null) } }
            }
        }
        return null
    }
    ...
    private fun saveLegacy(
        ...
        // createNewFile is the atomic reservation. An exists() check followed by
        // outputStream() lets concurrent same-name downloads both select and
        // truncate one path.
        val dest = claimLegacyDestination(dir, destDir, candidates) ?: return@runCatching null
        var committed = false
        try {
            ensureContinue(shouldContinue)
            ...
            if (!committed) dest.delete()
        }
    }.onFailure { if (it is CancellationException) throw it }.getOrNull()

    internal fun claimLegacyDestination(root: File, destinationDirectory: File, candidates: List<String>): File? {
        val canonicalRoot = root.canonicalFile
        val canonicalDirectory = destinationDirectory.canonicalFile
        if (canonicalDirectory != canonicalRoot && !DownloadPathPolicy.isContained(canonicalRoot, canonicalDirectory)) {
            return null
        }
        for (name in candidates) {
            val candidate = File(canonicalDirectory, name).canonicalFile
            if (!DownloadPathPolicy.isContained(canonicalRoot, candidate)) continue
    ...
    private fun ensureContinue(shouldContinue: () -> Boolean) {
        if (!shouldContinue()) throw CancellationException("save cancelled")
    }

    private fun verifyStillWanted(context: Context, savedPath: String?, shouldContinue: () -> Boolean): String? {
        if (savedPath == null) return null
        if (shouldContinue()) return savedPath
        delete(context, savedPath)
