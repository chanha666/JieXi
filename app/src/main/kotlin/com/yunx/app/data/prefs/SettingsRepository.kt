...
import com.yunx.app.data.download.DownloadPreset
...
class SettingsRepository(context: Context) {
    ...
    init {
        if (!prefs.getBoolean("single_task_max_v301_migrated", false)) {
            val current = prefs.getString("download_preset", null)
            val editor = prefs.edit().putBoolean("single_task_max_v301_migrated", true)
            if (current == null || current == DownloadPreset.BALANCED.name) {
                editor.putString("download_preset", DownloadPreset.SINGLE_TASK_MAX.name)
                    .putInt("max_concurrent_downloads", DownloadPreset.SINGLE_TASK_MAX.concurrentTasks)
                    .putInt("download_threads", DownloadPreset.SINGLE_TASK_MAX.connections)
            }
            editor.apply()
        }
    }
    ...
    var downloadPreset: DownloadPreset
        get() = DownloadPreset.from(prefs.getString("download_preset", DownloadPreset.SINGLE_TASK_MAX.name))
        set(value) {
            prefs.edit().putString("download_preset", value.name).apply()
            if (value != DownloadPreset.CUSTOM) {
                maxConcurrentDownloads = value.concurrentTasks
                downloadThreads = value.connections
            }
        }
    ...
    var allowRoaming: Boolean
        get() = prefs.getBoolean("allow_roaming", false)
        set(value) = prefs.edit().putBoolean("allow_roaming", value).apply()
    ...
    var pauseOnLowBattery: Boolean
        get() = prefs.getBoolean("pause_low_battery", true)
        set(value) = prefs.edit().putBoolean("pause_low_battery", value).apply()
    ...
    var historyRetentionDays: Int
        get() = prefs.getInt("history_retention_days", 30).coerceIn(0, 365)
        set(value) = prefs.edit().putInt("history_retention_days", value.coerceIn(0, 365)).apply()
    ...
    var themeColorMode: Int
        get() = prefs.getInt("theme_color_mode", 1)
        set(value) {
    ...
    /** 定制版自己的 GitHub 仓库，仅用于问题反馈；允许用户在设置中覆盖。 */
    var githubRepositoryUrl: String
        get() = prefs.getString("github_repository_url", BuildConfig.GITHUB_REPOSITORY_URL)
            .orEmpty().trim().removeSuffix("/issues/new").removeSuffix("/")
        set(value) = prefs.edit().putString("github_repository_url", normalizeGitHubRepository(value)).apply()

    fun githubIssuesUrl(): String? = githubRepositoryUrl.takeIf(String::isNotBlank)?.let { "$it/issues/new" }

    companion object {
        const val DEFAULT_DOWNLOAD_THREADS = 48
