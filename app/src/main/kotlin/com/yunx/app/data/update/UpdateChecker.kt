...
object UpdateChecker {
    ...
    suspend fun fetchLatestRelease(): Release? = withContext(Dispatchers.IO) {
        ...
        runCatching {
            val request = Request.Builder().url(endpoint).header("User-Agent", "JieXi-Android/4.0.0").get().build()
        ...
        val assets = buildList {
            json.optJSONArray("assets")?.let { array ->
                for (index in 0 until array.length()) {
                    ...
                    // APK updates are installable code. A signed manifest without
                    // a valid asset hash must never create an install button.
                    if (name.endsWith(".apk", true) && UpdateIntegrity.normalizeExpectedSha256(sha256) == null) continue
                    add(Asset(name, url, sha256.lowercase()))
                }
            }
        }
