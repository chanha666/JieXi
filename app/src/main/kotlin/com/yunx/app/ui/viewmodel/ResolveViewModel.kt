...
class ResolveViewModel(
    ...
    fun startResolve(link: String, pwd: String?) {
        ...
        viewModelScope.launch {
            ...
            if (credential.isBlank() && currentPlatform == SharePlatform.XUNLEI) {
                uiState = ResolveUiState.Error("迅雷官方接口要求授权，请先登录迅雷网盘")
                return@launch
            }
            ...
            repo.createSession(link, pwd, credential)
                .onSuccess { s ->
                    ...
                    historyDao.insert(
                        HistoryEntity(
                            link = ShareLinkParser.parse(link)?.let { parsed -> link.substringBefore('?') } ?: link,
                            title = s.title,
    ...
    fun goBack() {
        ...
        viewModelScope.launch {
            uiState = ResolveUiState.Loading
            val credential = currentCredential().orEmpty()
            if (credential.isBlank() && currentPlatform == SharePlatform.XUNLEI) return@launch
    ...
    fun fetchDownloadLink(file: ShareFile) {
        viewModelScope.launch {
            ...
            try {
                ...
                val credential = currentCredential().orEmpty()
                val anonymousDownload = currentPlatform == SharePlatform.UC || currentPlatform == SharePlatform.C139
                if (credential.isBlank() && !anonymousDownload) {
                    downloadError = "该平台允许免登录浏览，但官方下载需要登录"
                    return@launch
                }
