...
import com.yunx.desktop.util.DesktopLog as Log
...
class ChunkDownloader(private val clientProvider: () -> OkHttpClient) {
    ...
    suspend fun downloadChunk(
        ...
        repeat(attempts) { attempt ->
            ...
            val res = try {
                ...
                Log.w(TAG, "downloadChunk: task=$taskId 尝试${attempt + 1} IO异常: ${LogRedactor.line(e.message.orEmpty())}")
    ...
    suspend fun downloadFull(
        ...
        try {
            ...
            Log.w(TAG, "downloadFull: task=$taskId IO异常: ${LogRedactor.line(e.message.orEmpty())}")
            if (!isActive) throw CancellationException("下载被取消", e)
            false
        } finally {
            activeCalls[taskId]?.remove(call)
