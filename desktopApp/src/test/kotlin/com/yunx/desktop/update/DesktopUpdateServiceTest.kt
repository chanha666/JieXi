...
class DesktopUpdateServiceTest {
    ...
    fun `accepts author signed update manifest and rejects tampering`() {
        ...
        val payload = """{"tag_name":"v3.0.2","body":"test","assets":[{"name":"解析-3.0.2-安装版.exe","url":"https://downloads.example/app.exe","sha256":"${"a".repeat(64)}"}]}""".toByteArray()
        ...
        assertEquals("v3.0.2", release.version)
        assertEquals("解析-3.0.2-安装版.exe", DesktopUpdateService.windowsInstaller(release)?.name)
    ...
    fun `compares semantic version numbers numerically`() {
        assertEquals(1, DesktopUpdateService.compareVersions("v3.0.10", "3.0.2").coerceIn(-1, 1))
        assertEquals(0, DesktopUpdateService.compareVersions("3.0.2", "v3.0.2"))
    }
}
