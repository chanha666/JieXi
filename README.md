...
Windows 3.1.0 及以上版本可直接升级到 4.1.0，安装器只迁移三个任务数据文件且不会覆盖已有新数据。3.0.x 及更早版本会被安全阻止；请先备份 `%LOCALAPPDATA%\解析`，在 Windows 设置中卸载旧版，再安装 4.1.0。新版持久数据保存在 `%LOCALAPPDATA%\JieXi\Data`，正常卸载不会删除它。
